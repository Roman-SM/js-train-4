...
function compareCharCodeAtAndCodePointAt(string, index) {
  ...
  const charAtResult = string.charAt(index) // Використовуємо метод at(), щоб отримати символ з рядка на певній позиції, та запишемо результат в змінну atResult
  const atResult = string.at(index) // Використовуємо метод charCodeAt(), щоб отримати код символа з рядка на певній позиції, та запишемо результат в змінну charCodeAtResult
  const charCodeAtResult = string.charCodeAt(index) // Використовуємо метод codePointAt(), щоб отримати код символа з рядка на певній позиції, та запишемо результат в змінну codePointAtResult
  const codePointAtResult = string.codePointAt(index) // Повертаємо рядок в форматі  `charCodeAt:${charCodeAtResult}, codePointAt:${codePointAtResult}, charAt:${charAtResult}, at: ${atResult}`
...
function padString(original, length, padWith, side) {
  ...
  if (side === "start") { // Якщо "start", використовуємо padStart
  ...
  } else if (side === "end") { // Якщо "end", використовуємо padEnd
...
console.log('padString("123", 5, "0", "end")', 
  padString("123", 5, "0", "end")); // Виведе "12300"
console.log(
...
function convertCase(original, caseType) {
  ...
  return "Error: caseType should be either 'upper' or 'lower'"; // Якщо значення аргументу "caseType" не є ні "upper", ні "lower", повертаємо повідомлення "Error: caseType should be either 'upper' or 'lower'"
}
